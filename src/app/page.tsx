...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(reportsUrl).then((r) => r.json()),
    ])
  ...
  const myResolvedCount = reports.filter(
    (r) => r.status === "FIXED" || r.status === "VERIFIED_CLOSED"
  ).length;
  const myInReviewCount = reports.filter(
    (r) => r.status === "PENDING" || r.status === "TRIAGED" || r.status === "ASSIGNED"
  ).length;
    ...
    <AppLayout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full flex flex-col gap-6 bg-[#F8FAFC] min-h-screen">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
              ...
              OIL INDIA LIMITED · {isOfficer ? "OPERATIONS & RISK CENTER" : "FIELD SAFETY WORKSPACE"}
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0F172A] tracking-tight">
              {isOfficer
                ? "Safety Intelligence & SIF Precursor Hub"
                : `Welcome, ${user?.displayName || "Field Worker"}`}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              {isOfficer
                ? "Real-time precursor detection, closed-loop barrier tracking, and zero-fatality defense."
                : "Submit hazard observations, receive real-time verification feedback, and track hazard removals."}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            {isOfficer ? (
              ...
              <Link
                ...
                className="h-10 px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-2 shadow-xs transition-colors"
              >
                <span>My Safety Log ({myReportsCount})</span>
              </Link>
            )}
        ...
        {isOfficer ? (
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs flex flex-col justify-between">
              ...
              <div className="mt-3">
                ...
                <div className="text-[11px] font-semibold text-red-600 flex items-center gap-1 mt-1">
                  ...
                  <span>Immediate Officer Attention</span>
                </div>
              </div>
            </div>
          ...
          /* ─── EMPLOYEE VIEW: 3 WORKER STATUS CARDS ───────────── */
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  My Logged Observations
                </span>
                <div className="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-100 shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* ─── LEFT: OBSERVATIONS TABLE (8 cols) ─────────────── */}
          <div className="lg:col-span-8 bg-white border border-[#E2E8F0] rounded-2xl shadow-xs overflow-hidden flex flex-col justify-between">
              ...
              <div className="p-5 border-b border-[#E2E8F0] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-base font-bold text-[#0F172A]">
                    {isOfficer ? "Live Precursor & Incident Stream" : "My Recent Safety Observations"}
                  </h2>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {isOfficer
                      ? "Field observations analyzed by SCL Energy & Barrier decision tree."
                      : "Status of hazards and barrier issues you reported from the field."}
                  </p>
                </div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0] text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                      <th className="py-3 px-4">Observation / Field Note</th>
                  ...
                  <tbody className="divide-y divide-[#F1F5F9] text-slate-700">
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={5} className="py-12 text-center text-slate-400">
                          No reports recorded yet. Click &quot;+ Report Hazard&quot; to log one.
                        </td>
                      </tr>
                    ) : (
            ...
            <div className="p-4 border-t border-[#E2E8F0] flex items-center justify-between text-xs text-slate-500 bg-[#F8FAFC]/50">
              <div>
                Showing {Math.min(6, reports.length)} of {reports.length} records
              </div>
              <Link
                href="/reports"
              ...
              <>
                <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs">
                  ...
                  <p className="text-xs text-slate-500 mb-4">
                    Choose your preferred reporting mode from the field.
                  </p>

                  <div className="flex flex-col gap-2.5">
                    <Link
                      href="/report"
                      className="p-3 bg-blue-50/70 border border-blue-200 hover:bg-blue-100/70 rounded-xl flex items-center gap-3 transition-colors"
                    >
                      <span className="material-symbols-outlined text-[#2563EB]">mic</span>
                      <div>
                        <div className="text-xs font-bold text-slate-900">Voice Dictation</div>
                        <div className="text-[10px] text-slate-500">Speak in Assamese, Hindi, English</div>
                      </div>
                    </Link>

                    <Link
                      href="/report"
                      className="p-3 bg-slate-50 border border-slate-200 hover:bg-slate-100 rounded-xl flex items-center gap-3 transition-colors"
                    >
                      <span className="material-symbols-outlined text-slate-700">photo_camera</span>
                      <div>
                        <div className="text-xs font-bold text-slate-900">Photo with Face Blur</div>
                        <div className="text-[10px] text-slate-500">Auto-anonymizes colleague faces</div>
                      </div>
                    </Link>
                    ...
                    <Link
                      ...
                      className="p-3 bg-slate-50 border border-slate-200 hover:bg-slate-100 rounded-xl flex items-center gap-3 transition-colors"
                    >
                      <span className="material-symbols-outlined text-slate-700">touch_app</span>
                      <div>
                        <div className="text-xs font-bold text-slate-900">Tap-a-Hazard Grid</div>
                        <div className="text-[10px] text-slate-500">1-click hazard category selection</div>
                      </div>
                    </Link>
                  </div>
                </div>
                ...
                <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs">
                  ...
                  <p className="text-xs text-slate-500 mb-2">
                    Under OIL India Safety Policy:
                  </p>
                  <ul className="text-xs text-slate-600 list-disc pl-4 space-y-1">
                    <li>You have the right to <strong>Stop Work</strong> when a life-critical barrier is absent.</li>
                    <li>Reports are non-punitive and strictly confidential.</li>
                  </ul>
                </div>
              </>
            )}
